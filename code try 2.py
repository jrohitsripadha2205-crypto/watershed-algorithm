...
"""
    ...
    def init_tab2(self):
        ...
        self.params = {
            ...
            'step_5_topography': {'enabled': True, 'use_gradient': False, 'peak_threshold_ratio': 0.5,
                                   'gaussian_division_enabled': True, 'gaussian_sigma': 25.0},
    ...
    def tab2_load_roi(self):
        ...
        if file_path:
            ...
            if 'pipeline_params' in data:
                self.params = self._migrate_pipeline_params(data['pipeline_params'])
    ...
    def _migrate_pipeline_params(self, params):
        """Migrate a pipeline_params dict saved by an older version of this app
        (12-step pipeline, before the new Step 5 / Step 6 insertion) to the
        current 14-step key layout. Old keys step_5_markers..step_12_perspective
        map onto step_7_markers..step_14_perspective; new step 5/6 keys are
        filled in with defaults if absent so nothing is ever missing."""
        ...
        for old_key, new_key in legacy_map.items():
            ...
            elif old_key in migrated:
                migrated.pop(old_key)
        migrated.setdefault('step_5_topography',
                             {'enabled': True, 'use_gradient': False, 'peak_threshold_ratio': 0.5,
                              'gaussian_division_enabled': True, 'gaussian_sigma': 25.0})
        migrated.setdefault('step_6_topo_watershed',
                             {'enabled': True, 'min_distance': 5, 'compactness': 0.0, 'connectivity': 1})
        # Migrate from the short-lived intermediate key layout used before this revision
        if 'step_5_gaussian_division' in migrated:
            old5 = migrated.pop('step_5_gaussian_division')
            migrated['step_5_topography']['gaussian_division_enabled'] = old5.get('enabled', True)
            migrated['step_5_topography']['gaussian_sigma'] = old5.get('sigma', 25.0)
        if 'step_6_batch_segmentation' in migrated:
            migrated.pop('step_6_batch_segmentation')
        return migrated
    ...
    def process_all_steps(self):
        ...
        # Step 5: Topography (surface generation for watershed flooding)  [NEW]
        # Builds the "elevation" surface the Step 6 watershed floods, using
        # either a morphological-gradient (ridge) surface or a distance-
        # transform (valley) surface computed from a working foreground mask.
        # Optionally illumination-normalizes the source image first via
        # Gaussian Division (divide by a heavily-blurred copy of itself).
        p5 = self.params['step_5_topography']
        if p5['enabled']:
            if p5['gaussian_division_enabled']:
                sigma = max(1.0, float(p5['gaussian_sigma']))
                gray_f = gray.astype(np.float32)
                illumination = cv2.GaussianBlur(gray_f, (0, 0), sigmaX=sigma, sigmaY=sigma)
                illumination[illumination < 1e-6] = 1e-6
                topo_source = np.clip((gray_f / illumination) * 128.0, 0, 255).astype(np.uint8)
            else:
                topo_source = gray.copy()

            peak_ratio = float(np.clip(p5['peak_threshold_ratio'], 0.0, 0.95))
            thresh_val = int(peak_ratio * 255)
            _, topo_fg_mask = cv2.threshold(topo_source, thresh_val, 255, cv2.THRESH_BINARY)

            if p5['use_gradient']:
                kernel3 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
                flooding_surface = cv2.morphologyEx(topo_source, cv2.MORPH_GRADIENT, kernel3).astype(np.float32)
            else:
                flooding_surface = -cv2.distanceTransform(topo_fg_mask, cv2.DIST_L2, 5)

            topo_display = cv2.cvtColor(topo_source, cv2.COLOR_GRAY2RGB)
            overlay = topo_display.copy()
            overlay[topo_fg_mask > 0] = [0, 200, 0]
            topo_display = cv2.addWeighted(topo_display, 0.6, overlay, 0.4, 0)
        else:
            topo_source = gray.copy()
            topo_fg_mask = (gray > 0).astype(np.uint8) * 255
            flooding_surface = -cv2.distanceTransform(topo_fg_mask, cv2.DIST_L2, 5)
            topo_display = cv2.cvtColor(topo_source, cv2.COLOR_GRAY2RGB)

        self.step_images[5] = topo_display
        self.step_data['topo_source'] = topo_source
        self.step_data['topo_fg_mask'] = topo_fg_mask
        self.step_data['flooding_surface'] = flooding_surface

        # Step 6: Topographic Watershed  [NEW]
        # Seeds markers as local maxima of the distance-transformed
        # foreground mask (classic skimage watershed recipe), then floods
        # the Step 5 surface from those markers to split touching defects.
        p6 = self.params['step_6_topo_watershed']
        if p6['enabled']:
            min_distance = max(1, int(p6['min_distance']))
            compactness = max(0.0, float(p6['compactness']))
            connectivity = max(1, int(p6['connectivity']))

            dist_for_peaks = cv2.distanceTransform(topo_fg_mask, cv2.DIST_L2, 5)
            peak_ratio = float(np.clip(p5['peak_threshold_ratio'], 0.0, 0.95))
            coords = peak_local_max(dist_for_peaks, min_distance=min_distance,
                                     threshold_rel=peak_ratio, labels=topo_fg_mask)
            markers_mask = np.zeros_like(topo_fg_mask, dtype=bool)
            if len(coords) > 0:
                markers_mask[tuple(coords.T)] = True
            markers6, _ = ndi.label(markers_mask)

            conn_footprint = disk(connectivity) if connectivity > 1 else None
            if markers6.max() > 0:
                seg_labels = watershed(flooding_surface, markers6, mask=topo_fg_mask,
                                        compactness=compactness, connectivity=conn_footprint)
                new_segmented_mask = (seg_labels > 0).astype(np.uint8) * 255
            else:
                new_segmented_mask = topo_fg_mask.copy()

            batch_seg_display = cv2.cvtColor(topo_source, cv2.COLOR_GRAY2RGB)
            batch_seg_display[new_segmented_mask > 0] = [255, 0, 0]
        else:
            new_segmented_mask = topo_fg_mask.copy()
            batch_seg_display = cv2.cvtColor(topo_source, cv2.COLOR_GRAY2RGB)

        self.step_images[6] = batch_seg_display
        self.step_data['new_segmented_mask'] = new_segmented_mask
        ...
        # Step 8: Watershed Application  (was Step 7)
        ...
        if not self.params['step_7_markers']['enabled'] and not self.params['step_8_watershed']['enabled']:
            # CRITICAL REQUIREMENT: when both the (old) Markers and Watershed steps
            # are skipped, forward the segmented output produced by the newly
            # inserted Step 5 (Topography) / Step 6 (Topographic Watershed)
            # instead of a bare passthrough, so the pipeline never
            # receives an empty / missing image.
            binary_seg = (self.step_data['new_segmented_mask'] > 0).astype(np.uint8)
        elif self.params['step_7_markers']['enabled']:
            high_thresh = self.params['step_7_markers']['high_threshold']
            binary_seg = (segmentation > high_thresh).astype(np.uint8)
        else:
            binary_seg = (segmentation > 0).astype(np.uint8)

        # Step 9: Morphological Close  (was Step 8)
        ...
        # Step 10: Morphological Open  (was Step 9)
        ...
        # Step 11: Fill Holes  (was Step 10)
        ...
        # Step 12: Label Connected Components  (was Step 11)
        ...
        # Step 13: Final Result with Colored Segments  (was Step 12)
        ...
        # Step 14: Perspective Warp  (was Step 13)
    ...
    def display_current_step(self):
        ...
        step_names = [
            ...
            "Step 5: Topography (Gradient / Distance Surface + Gaussian Division)",
            "Step 6: Topographic Watershed",
        ...
        step_descriptions = [
            ...
            "Builds the flooding surface for watershed: gradient (ridges) or distance-transform (valleys) "
            "from a foreground mask, with optional Gaussian Division illumination normalization first.",
            "Seeds markers as local maxima of the foreground distance transform, then floods the Step 5 "
            "surface to split touching defects. Produces the segmented mask that downstream steps use "
            "automatically if Markers/Watershed below are skipped.",
    ...
    def update_parameter_controls(self):
        ...
        elif step == 5:  # Topography
            self.create_step5_controls()
        elif step == 6:  # Topographic Watershed
    ...
    def create_step5_controls(self):
        """Create controls for Step 5: Topography (surface generation for watershed)"""
        ...
        # Use Gradient toggle
        grad_var = tk.BooleanVar(value=self.params['step_5_topography']['use_gradient'])
        ttk.Checkbutton(self.params_frame, text="Use Gradient Topography (ridges instead of valleys)",
                        variable=grad_var,
                        command=lambda: self.update_step5_topo_flags(grad_var, None)).pack(anchor='w', pady=5)

        # Gaussian Division toggle
        gdiv_var = tk.BooleanVar(value=self.params['step_5_topography']['gaussian_division_enabled'])
        ttk.Checkbutton(self.params_frame, text="Enable Gaussian Division (illumination normalization)",
                        variable=gdiv_var,
                        command=lambda: self.update_step5_topo_flags(None, gdiv_var)).pack(anchor='w', pady=5)

        ttk.Label(self.params_frame, text="Gaussian Sigma (illumination blur radius):").pack(anchor='w', pady=(10, 2))
        sigma_frame = ttk.Frame(self.params_frame)
        sigma_frame.pack(fill='x', pady=2)
        sigma_var = tk.DoubleVar(value=self.params['step_5_topography']['gaussian_sigma'])
        ttk.Scale(sigma_frame, from_=1, to=150, orient='horizontal', variable=sigma_var,
                  command=lambda v: self.update_step5_sigma(sigma_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        sigma_spinbox = ttk.Spinbox(sigma_frame, from_=1, to=150, textvariable=sigma_var,
                                    width=6, command=lambda: self.update_step5_sigma(sigma_var))
        sigma_spinbox.pack(side='right')
        sigma_spinbox.bind('<Return>', lambda e: self.update_step5_sigma(sigma_var))
        sigma_spinbox.bind('<FocusOut>', lambda e: self.update_step5_sigma(sigma_var))

        ttk.Label(self.params_frame, text="Peak Threshold Ratio (foreground cutoff, 0-0.95):").pack(anchor='w', pady=(10, 2))
        ratio_frame = ttk.Frame(self.params_frame)
        ratio_frame.pack(fill='x', pady=2)
        ratio_var = tk.DoubleVar(value=self.params['step_5_topography']['peak_threshold_ratio'])
        ttk.Scale(ratio_frame, from_=0.0, to=0.95, orient='horizontal', variable=ratio_var,
                  command=lambda v: self.update_step5_ratio(ratio_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        ratio_spinbox = ttk.Spinbox(ratio_frame, from_=0.0, to=0.95, increment=0.05, textvariable=ratio_var,
                                    width=6, command=lambda: self.update_step5_ratio(ratio_var))
        ratio_spinbox.pack(side='right')
        ratio_spinbox.bind('<Return>', lambda e: self.update_step5_ratio(ratio_var))
        ratio_spinbox.bind('<FocusOut>', lambda e: self.update_step5_ratio(ratio_var))
        self.param_widgets = {'grad_var': grad_var, 'gdiv_var': gdiv_var, 'sigma_var': sigma_var, 'ratio_var': ratio_var}

        ttk.Label(self.params_frame,
                  text="Note: builds the flooding surface used by Step 6's watershed  either a "
                       "morphological-gradient (ridge) surface or a distance-transform (valley) surface, "
                       "computed from a foreground mask thresholded at Peak Threshold Ratio. Gaussian "
                       "Division normalizes uneven illumination before thresholding.",
                  wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)

    def create_step6_controls(self):
        """Create controls for Step 6: Topographic Watershed"""
        skip_var = tk.BooleanVar(value=not self.params['step_6_topo_watershed']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_6_topo_watershed', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        ttk.Label(self.params_frame, text="Min Distance (peak marker separation):").pack(anchor='w', pady=(5, 2))
        mdist_frame = ttk.Frame(self.params_frame)
        mdist_frame.pack(fill='x', pady=2)
        mdist_var = tk.IntVar(value=self.params['step_6_topo_watershed']['min_distance'])
        ttk.Scale(mdist_frame, from_=1, to=100, orient='horizontal', variable=mdist_var,
                  command=lambda v: self.update_step6_params(mdist_var, None, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        mdist_spinbox = ttk.Spinbox(mdist_frame, from_=1, to=100, textvariable=mdist_var,
                                    width=6, command=lambda: self.update_step6_params(mdist_var, None, None))
        mdist_spinbox.pack(side='right')
        mdist_spinbox.bind('<Return>', lambda e: self.update_step6_params(mdist_var, None, None))
        mdist_spinbox.bind('<FocusOut>', lambda e: self.update_step6_params(mdist_var, None, None))

        ttk.Label(self.params_frame, text="Compactness (shape regularization):").pack(anchor='w', pady=(10, 2))
        comp_frame = ttk.Frame(self.params_frame)
        comp_frame.pack(fill='x', pady=2)
        comp_var = tk.DoubleVar(value=self.params['step_6_topo_watershed']['compactness'])
        ttk.Scale(comp_frame, from_=0.0, to=5.0, orient='horizontal', variable=comp_var,
                  command=lambda v: self.update_step6_params(None, comp_var, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        comp_spinbox = ttk.Spinbox(comp_frame, from_=0.0, to=5.0, increment=0.05, textvariable=comp_var,
                                   width=6, command=lambda: self.update_step6_params(None, comp_var, None))
        comp_spinbox.pack(side='right')
        comp_spinbox.bind('<Return>', lambda e: self.update_step6_params(None, comp_var, None))
        comp_spinbox.bind('<FocusOut>', lambda e: self.update_step6_params(None, comp_var, None))
        ...
        conn_var = tk.IntVar(value=self.params['step_6_topo_watershed']['connectivity'])
        ttk.Scale(conn_frame, from_=1, to=10, orient='horizontal', variable=conn_var,
                  command=lambda v: self.update_step6_params(None, None, conn_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        conn_spinbox = ttk.Spinbox(conn_frame, from_=1, to=10, textvariable=conn_var,
                                   width=6, command=lambda: self.update_step6_params(None, None, conn_var))
        conn_spinbox.pack(side='right')
        conn_spinbox.bind('<Return>', lambda e: self.update_step6_params(None, None, conn_var))
        conn_spinbox.bind('<FocusOut>', lambda e: self.update_step6_params(None, None, conn_var))
        self.param_widgets = {'mdist_var': mdist_var, 'comp_var': comp_var, 'conn_var': conn_var}

        ttk.Label(self.params_frame,
                  text="Note: seeds markers as local maxima of the Step 5 foreground's distance "
                       "transform, then floods the Step 5 surface from those markers to split "
                       "touching defects. If Markers (Step 7) and Watershed (Step 8) below are both "
                       "skipped, this step's segmented mask is forwarded automatically so the "
                       "pipeline never receives an empty image.",
                  wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)
    ...
    def update_step5_topo_flags(self, grad_var, gdiv_var):
        """Update step 5 Use-Gradient / Gaussian-Division toggles"""
        if grad_var is not None:
            self.params['step_5_topography']['use_gradient'] = grad_var.get()
        if gdiv_var is not None:
            self.params['step_5_topography']['gaussian_division_enabled'] = gdiv_var.get()
        self.process_all_steps()
        self.display_current_step()

    def update_step5_sigma(self, sigma_var):
        """Update step 5 Gaussian Division sigma"""
        try:
            value = max(1.0, min(150.0, float(sigma_var.get())))
            sigma_var.set(value)
            self.params['step_5_topography']['gaussian_sigma'] = value
            self.process_all_steps()
            self.display_current_step()
        except (ValueError, tk.TclError):
            sigma_var.set(self.params['step_5_topography']['gaussian_sigma'])

    def update_step5_ratio(self, ratio_var):
        """Update step 5 peak threshold ratio (also used by Step 6's marker seeding)"""
        try:
            value = max(0.0, min(0.95, float(ratio_var.get())))
            ratio_var.set(value)
    ...
    def update_step6_params(self, mdist_var, comp_var, conn_var):
        """Update step 6 topographic watershed min_distance / compactness / connectivity"""
        try:
            if mdist_var is not None:
                value = max(1, min(100, int(mdist_var.get())))
                mdist_var.set(value)
                self.params['step_6_topo_watershed']['min_distance'] = value
            if comp_var is not None:
                value = max(0.0, min(5.0, float(comp_var.get())))
                comp_var.set(value)
            ...
            if mdist_var is not None:
                mdist_var.set(self.params['step_6_topo_watershed']['min_distance'])
            if comp_var is not None:
                comp_var.set(self.params['step_6_topo_watershed']['compactness'])
            if conn_var is not None:
                conn_var.set(self.params['step_6_topo_watershed']['connectivity'])
    ...
    def tab3_process_single_image(self, image_path, defects_folder, non_defects_folder, filename):
        ...
        if self.tab3_config_data and 'pipeline_params' in self.tab3_config_data:
            params = self._migrate_pipeline_params(self.tab3_config_data['pipeline_params'])
        else:
            params = {
                'step_2_rotate_flip': {'enabled': True, 'rotate': 0, 'flip_h': False, 'flip_v': False},
                'step_3_grayscale': {'enabled': True},
        ...
        # Step 5: Topography (surface generation for watershed flooding)  [NEW]
        p5 = params.get('step_5_topography', {})
        if p5.get('enabled', True):
            if p5.get('gaussian_division_enabled', True):
                sigma = max(1.0, float(p5.get('gaussian_sigma', 25.0)))
                gray_f = gray.astype(np.float32)
                illumination = cv2.GaussianBlur(gray_f, (0, 0), sigmaX=sigma, sigmaY=sigma)
                illumination[illumination < 1e-6] = 1e-6
                topo_source = np.clip((gray_f / illumination) * 128.0, 0, 255).astype(np.uint8)
            ...
            peak_ratio = float(np.clip(p5.get('peak_threshold_ratio', 0.5), 0.0, 0.95))
            thresh_val = int(peak_ratio * 255)
            _, topo_fg_mask = cv2.threshold(topo_source, thresh_val, 255, cv2.THRESH_BINARY)

            if p5.get('use_gradient', False):
                kernel3 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
                flooding_surface = cv2.morphologyEx(topo_source, cv2.MORPH_GRADIENT, kernel3).astype(np.float32)
            else:
                flooding_surface = -cv2.distanceTransform(topo_fg_mask, cv2.DIST_L2, 5)
        ...
        # Step 6: Topographic Watershed  [NEW]
        p6 = params.get('step_6_topo_watershed', {})
        if p6.get('enabled', True):
            min_distance = max(1, int(p6.get('min_distance', 5)))
            compactness = max(0.0, float(p6.get('compactness', 0.0)))
            connectivity = max(1, int(p6.get('connectivity', 1)))
        ...
        if not params.get('step_7_markers', {}).get('enabled', True) and \
           not params.get('step_8_watershed', {}).get('enabled', True):
            # CRITICAL REQUIREMENT: both Markers (step 7) and Watershed (step 8)
            # skipped -> forward the segmented mask produced by the new Step 5/6
            # (Topography + Topographic Watershed) instead of an
            # empty / passthrough image.
            binary_seg = (new_segmented_mask > 0).astype(np.uint8)
        elif params.get('step_7_markers', {}).get('enabled', True):
            # binary_seg = (segmentation > high_thresh).astype(np.uint8)
            high_thresh = params['step_7_markers'].get('high_threshold', 150)
            binary_seg = (segmentation > high_thresh).astype(np.uint8)
        else:
