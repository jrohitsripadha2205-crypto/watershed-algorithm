...
"""
    ...
    def init_tab2(self):
        ...
        self.params = {
            ...
            'step_5_topography': {'enabled': True, 'use_gradient': False, 'peak_threshold_ratio': 0.5,
                'gaussian_division_enabled': True,
                'gaussian_sigma': 50.0},
            'step_6_topo_watershed': {'enabled': True, 'min_distance': 5, 'compactness': 0.0, 'connectivity': 1},
            'step_7_markers': {'enabled': False, 'low_threshold': 90, 'high_threshold': 220},
    ...
    def update_parameter_controls(self):
        ...
        elif step == 5:  # Topography Surface
            self.create_step5_controls()
        elif step == 6:  # Marker Separation / Compactness / Connectivity
    ...
    def create_step5_controls(self):
        """Create controls for Step 5: Topography Surface Style & Peak Threshold Ratio"""
        ...
        # Topography surface style
        ttk.Label(self.params_frame, text="Topography Surface Style:").pack(anchor='w', pady=(5, 2))
        gradient_var = tk.BooleanVar(value=self.params['step_5_topography']['use_gradient'])
        style_frame = ttk.Frame(self.params_frame)
        style_frame.pack(fill='x', pady=2)
        ttk.Radiobutton(style_frame, text="Distance Transform", variable=gradient_var,
                        value=False,
                        command=lambda: self.update_step5_topography(gradient_var, None)).pack(anchor='w')
        ttk.Radiobutton(style_frame, text="Morphological Gradient", variable=gradient_var,
                        value=True,
                        command=lambda: self.update_step5_topography(gradient_var, None)).pack(anchor='w')

        # Foreground peak detection cutoff threshold ratio
        ttk.Label(self.params_frame, text="Foreground Peak Cutoff Threshold Ratio:").pack(anchor='w', pady=(10, 2))
        ...
        ttk.Scale(ratio_frame, from_=0.0, to=1.0, orient='horizontal', variable=ratio_var,
                  command=lambda v: self.update_step5_topography(None, ratio_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        ratio_spinbox = ttk.Spinbox(ratio_frame, from_=0.0, to=1.0, increment=0.01, textvariable=ratio_var,
                                    width=6, command=lambda: self.update_step5_topography(None, ratio_var))
        ratio_spinbox.pack(side='right')
        ratio_spinbox.bind('<Return>', lambda e: self.update_step5_topography(None, ratio_var))
        ratio_spinbox.bind('<FocusOut>', lambda e: self.update_step5_topography(None, ratio_var))
        self.param_widgets = {'gradient_var': gradient_var, 'ratio_var': ratio_var}

        ttk.Label(self.params_frame,
                  text="Note: Sets the topography (flooding) surface used by the watershed "
                       "engine and the ratio of the intensity range used to cut off the "
                       "foreground peak mask.",
                 wraplength=280, font=('Arial', 8)).pack(anchor='w', pady=10)

        # Gaussian Division preprocessing controls
        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        gaussian_div_var = tk.BooleanVar(
            value=self.params['step_5_topography'].get('gaussian_division_enabled', True)
        )
        ttk.Checkbutton(
            self.params_frame,
            text="Enable Gaussian Division Preprocessing",
            variable=gaussian_div_var,
            command=lambda: self.update_step5_gaussian_division(gaussian_div_var, None)
        ).pack(anchor='w', pady=5)

        ttk.Label(self.params_frame, text="Gaussian Division Sigma:").pack(
            anchor='w', pady=(10, 2)
        )
        ...
        sigma_var = tk.DoubleVar(
            value=self.params['step_5_topography'].get('gaussian_sigma', 50.0)
        )
        ttk.Scale(
            sigma_frame, from_=0.5, to=200.0, orient='horizontal',
            variable=sigma_var,
            command=lambda v: self.update_step5_gaussian_division(None, sigma_var)
        ).pack(side='left', fill='x', expand=True, padx=(0, 5))

        sigma_spinbox = ttk.Spinbox(
            sigma_frame, from_=0.5, to=200.0, increment=0.5,
            textvariable=sigma_var, width=7,
            command=lambda: self.update_step5_gaussian_division(None, sigma_var)
        )
        sigma_spinbox.pack(side='right')
        sigma_spinbox.bind(
            '<Return>',
            lambda e: self.update_step5_gaussian_division(None, sigma_var)
        )
        sigma_spinbox.bind(
            '<FocusOut>',
            lambda e: self.update_step5_gaussian_division(None, sigma_var)
    ...
    def create_step6_controls(self):
        """Create controls for Step 6: Marker Separation, Compactness & Connectivity"""
        ...
        dist_frame = ttk.Frame(self.params_frame)
        dist_frame.pack(fill='x', pady=2)
        dist_var = tk.IntVar(value=self.params['step_6_topo_watershed']['min_distance'])
        ttk.Scale(dist_frame, from_=1, to=30, orient='horizontal', variable=dist_var,
                  command=lambda v: self.update_step6_topo_watershed(dist_var, None, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        dist_spinbox = ttk.Spinbox(dist_frame, from_=1, to=30, textvariable=dist_var,
                                   width=6, command=lambda: self.update_step6_topo_watershed(dist_var, None, None))
        dist_spinbox.pack(side='right')
        dist_spinbox.bind('<Return>', lambda e: self.update_step6_topo_watershed(dist_var, None, None))
        dist_spinbox.bind('<FocusOut>', lambda e: self.update_step6_topo_watershed(dist_var, None, None))

        # Watershed compactness regularization
        ...
        compactness_var = tk.DoubleVar(value=self.params['step_6_topo_watershed']['compactness'])
        ttk.Scale(comp_frame, from_=0.0, to=1.0, orient='horizontal', variable=compactness_var,
                  command=lambda v: self.update_step6_topo_watershed(None, compactness_var, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        comp_spinbox = ttk.Spinbox(comp_frame, from_=0.0, to=1.0, increment=0.01, textvariable=compactness_var,
                                   width=6, command=lambda: self.update_step6_topo_watershed(None, compactness_var, None))
        comp_spinbox.pack(side='right')
        comp_spinbox.bind('<Return>', lambda e: self.update_step6_topo_watershed(None, compactness_var, None))
        comp_spinbox.bind('<FocusOut>', lambda e: self.update_step6_topo_watershed(None, compactness_var, None))

        # Connectivity footprint neighborhood metric
        ttk.Label(self.params_frame, text="Connectivity Footprint (Neighborhood):").pack(anchor='w', pady=(10, 2))
        conn_frame = ttk.Frame(self.params_frame)
        conn_frame.pack(fill='x', pady=2)
        connectivity_var = tk.IntVar(value=self.params['step_6_topo_watershed']['connectivity'])
        ttk.Scale(conn_frame, from_=1, to=8, orient='horizontal', variable=connectivity_var,
                  command=lambda v: self.update_step6_topo_watershed(None, None, connectivity_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        conn_spinbox = ttk.Spinbox(conn_frame, from_=1, to=8, textvariable=connectivity_var,
                                   width=6, command=lambda: self.update_step6_topo_watershed(None, None, connectivity_var))
        conn_spinbox.pack(side='right')
        conn_spinbox.bind('<Return>', lambda e: self.update_step6_topo_watershed(None, None, connectivity_var))
        conn_spinbox.bind('<FocusOut>', lambda e: self.update_step6_topo_watershed(None, None, connectivity_var))
        self.param_widgets = {'dist_var': dist_var, 'compactness_var': compactness_var, 'connectivity_var': connectivity_var}

        ttk.Label(self.params_frame,
                  text="Note: Minimum separation controls how far apart adjacent peak markers "
                       "must be before merging. Compactness regularizes region shape. "
                       "Connectivity sets the neighborhood footprint used by the watershed engine.",
                 wraplength=280, font=('Arial', 8)).pack(anchor='w', pady=10)             
    ...
    def tab3_process_single_image(self, image_path, defects_folder, non_defects_folder, filename):
        ...
        if topo_params.get('enabled', True):
            ...
            peak_cutoff = int(round(peak_ratio * 255))
            _, filtered_mask = cv2.threshold(gray, peak_cutoff, 255, cv2.THRESH_BINARY)
            if use_gradient:
                flooding_surface = cv2.morphologyEx(
            ...
            _, filtered_mask = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            flooding_surface = gray.astype(np.float32)
        ...
        if topo_ws_params.get('enabled', True):
            ...
            selem = disk(min_distance)
            ret, topo_markers = cv2.connectedComponents(filtered_mask)
            topo_markers = topo_markers + 1
            ...
            segmentation_labels = _skimage_watershed(
                ...
                mask=filtered_mask, compactness=compactness,
                connectivity=conn_footprint
            )
        else:
            segmentation_labels = filtered_mask.copy()    
