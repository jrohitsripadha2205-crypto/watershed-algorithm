...
# ============================================================================
# Watershed Engine Optimization (Step 6) support - ported from Batch_processesing.py
# scikit-image is used for compactness / connectivity-aware watershed. Import
# is guarded so the application still runs (with Step 6 gracefully degraded
# to a plain-connectivity fallback) on machines where scikit-image isn't
# installed.
# ============================================================================
try:
...
"""
    ...
    def init_tab2(self):
        ...
        self.params = {
            ...
            'step_5_topography': {'enabled': True, 'mode': 0, 'fg_threshold': 0.50},
            'step_6_watershed_opt': {'enabled': True, 'min_distance': 5, 'compactness': 0.0, 'connectivity': 1},
        ...
        self.progress_label = tk.Label(nav_bar, text="Step 0 / 14",
                                       bg=self.PANEL, fg=self.FG2,
                                       font=(self.FONT, 10))
    ...
    def tab2_load_roi(self):
        ...
        if file_path:
            ...
            if 'pipeline_params' in data:
                self.params = self._migrate_legacy_pipeline_params(data['pipeline_params'])
    ...
    def process_all_steps(self):
        ...
        # Step 5: Topography & Distance Analytics  (NEW)
        topo_surface, topo_peak_mask, topo_display_rgb = self._compute_topography_analytics(
            gray, self.params['step_5_topography']
        )
        self.step_images[5] = topo_display_rgb
        self.step_data['topography_surface'] = topo_surface
        self.step_data['topo_peak_mask'] = topo_peak_mask

        # Step 6: Watershed Engine Optimization  (NEW)
        eng_markers, eng_segmentation, eng_peak_mask, eng_display_rgb = self._compute_watershed_engine(
            gray, topo_surface, topo_peak_mask, self.params['step_6_watershed_opt']
        )
        self.step_images[6] = eng_display_rgb
        self.step_data['engine_markers'] = eng_markers
        self.step_data['engine_segmentation'] = eng_segmentation
        self.step_data['engine_peak_mask'] = eng_peak_mask

        # Step 7: Histogram-based Markers  (was Step 5)
        if self.params['step_7_markers']['enabled']:
            ...
            # Real feed-forward from Step 6: engine-detected foreground peaks
            # (shaped by min separation distance / compactness / connectivity)
            # are folded into the marker map as additional high-confidence
            # foreground seeds, so Step 6's parameters genuinely influence the
            # existing Markers step rather than only Step 6's own preview.
            if eng_markers is not None:
                engine_fg = eng_markers > 1
                markers[engine_fg] = np.maximum(
                    markers[engine_fg],
                    (gray[engine_fg].astype(np.int32) + 1)
                )
        ...
        # Step 8: Watershed Application  (was Step 6)
        if self.params['step_8_watershed']['enabled']:
            # Real feed-forward from Step 5: use the computed topography
            # surface (distance transform / morphological gradient) as the
            # flooding elevation map when available, instead of only the
            # plain Sobel elevation map.
            if topo_surface is not None:
                elevation_map_uint8 = cv2.normalize(topo_surface, None, 0, 255,
        ...
        # Step 9: Morphological Close  (was Step 7)
        ...
        # Step 10: Morphological Open  (was Step 8)
        ...
        # Step 12: Label Connected Components  (was Step 10)
        ...
        # Step 13: Final Result with Colored Segments  (was Step 11)
        ...
        # Step 14: Perspective Warp  (was Step 12)
    ...
    # ==================== NEW STEP 5 / STEP 6 ENGINE HELPERS ====================
    # Shared by both Single Image (process_all_steps) and Batch Processing
    # (tab3_process_single_image) so the two pipelines stay identical.

    @staticmethod
    def _migrate_legacy_pipeline_params(params):
        """Backward-compatibility for pre-14-step (12-step) saved configs.
        Old key names (step_5_markers .. step_12_perspective) referred to the
        old step numbering; remap them to the new 14-step key names and
        supply defaults for the new Step 5 / Step 6 parameters when missing.
        Newer 14-step configs are returned unchanged.
        """
        if not isinstance(params, dict):
            return params

        legacy_key_map = {
            'step_5_markers':     'step_7_markers',
            'step_6_watershed':   'step_8_watershed',
            'step_7_morph_close': 'step_9_morph_close',
            'step_8_morph_open':  'step_10_morph_open',
            'step_9_fillholes':   'step_11_fillholes',
            'step_10_labeling':   'step_12_labeling',
            'step_11_final':      'step_13_final',
            'step_12_perspective': 'step_14_perspective',
        }
        is_legacy = 'step_7_markers' not in params and 'step_5_markers' in params
        if is_legacy:
            migrated = {}
    ...
    @staticmethod
    def _compute_topography_analytics(gray, step5_params):
        """Step 5: Topography & Distance Analytics.
        Ported from Batch_processesing.py Tab 2 (Topography Surface Mode +
        Foreground Peak Detection Cutoff Threshold Ratio).
        Returns (topography_surface[float32], topo_peak_mask[uint8 0/255], display_rgb[uint8 HxWx3]).
        """
        kernel3 = np.ones((3, 3), np.uint8)
        mode = int(step5_params.get('mode', 0))          # 0 = Distance Transform, 1 = Morphological Gradient
        fg_ratio = float(step5_params.get('fg_threshold', 0.50))

        if step5_params.get('enabled', True):
            _, base_mask = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            base_mask = cv2.morphologyEx(base_mask, cv2.MORPH_OPEN, kernel3, iterations=1)

            if mode > 0:
                # Edge Morphological Gradient (Ridges)
                topo_surface = cv2.morphologyEx(gray, cv2.MORPH_GRADIENT, kernel3).astype(np.float32)
                response = topo_surface
                cmap = cv2.COLORMAP_MAGMA
            else:
                # Distance Transform (Valleys)
                dist = cv2.distanceTransform(base_mask, cv2.DIST_L2, 5)
                topo_surface = -dist
                response = dist
                cmap = cv2.COLORMAP_VIRIDIS

            resp_norm = cv2.normalize(response, None, 0.0, 1.0, cv2.NORM_MINMAX)
            topo_peak_mask = ((resp_norm >= fg_ratio).astype(np.uint8)) * 255

            norm_disp = cv2.normalize(response, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            display_bgr = cv2.applyColorMap(norm_disp, cmap)
            display_rgb = cv2.cvtColor(display_bgr, cv2.COLOR_BGR2RGB)
        else:
            # Skipped: pass forward a valid default surface/mask so downstream
            # steps (and Step 6) never see None / invalid state.
            topo_surface = gray.astype(np.float32)
            topo_peak_mask = None
            display_rgb = cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB)

        return topo_surface, topo_peak_mask, display_rgb

    @staticmethod
    def _compute_watershed_engine(gray, topo_surface, topo_peak_mask, step6_params):
        """Step 6: Watershed Engine Optimization.
        Ported from Batch_processesing.py Tab 3 (Peak Marker Minimum Separation
        Distance, Watershed Compactness, Connectivity Radius).
        Returns (engine_markers[int32 or None], engine_segmentation[int32 or None],
                 engine_peak_mask[uint8 0/255 or None], display_rgb[uint8 HxWx3]).
        """
        min_dist = int(step6_params.get('min_distance', 5))
        compactness = float(step6_params.get('compactness', 0.0))
        connectivity = int(step6_params.get('connectivity', 1))

        if not step6_params.get('enabled', True) or not _SKIMAGE_AVAILABLE:
            # Skipped (or scikit-image unavailable): preserve/pass the most
            # recent valid data instead of leaving None state where it would
            # break downstream consumers that expect a mask.
            return None, None, topo_peak_mask, cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB)

        peak_mask = topo_peak_mask
        if peak_mask is None or cv2.countNonZero(peak_mask) == 0:
            _, peak_mask = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

        surface = topo_surface if topo_surface is not None else gray.astype(np.float32)

        # Enforce minimum peak separation distance by eroding seed candidates
        # with a structuring element sized from the parameter.
        if min_dist > 1:
            er_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (min_dist, min_dist))
            seed_mask = cv2.erode(peak_mask, er_kernel, iterations=1)
            if cv2.countNonZero(seed_mask) == 0:
                seed_mask = peak_mask.copy()
        else:
            seed_mask = peak_mask.copy()

        ret, eng_markers = cv2.connectedComponents(seed_mask)
        eng_markers = eng_markers + 1
        unknown = cv2.subtract(cv2.dilate(peak_mask, np.ones((3, 3), np.uint8), iterations=1), peak_mask)
        eng_markers[unknown == 255] = 0

        conn_footprint = sk_disk(int(connectivity)) if connectivity > 1 else None
        try:
            eng_segmentation = sk_watershed(surface, eng_markers, mask=(peak_mask > 0),
                                            compactness=compactness, connectivity=conn_footprint)
        except Exception:
            eng_segmentation = eng_markers.copy()

        seg_norm = cv2.normalize(eng_segmentation.astype(np.float32), None, 0, 255,
                                 cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        display_bgr = cv2.applyColorMap(seg_norm, cv2.COLORMAP_JET)
        display_rgb = cv2.cvtColor(display_bgr, cv2.COLOR_BGR2RGB)

        return eng_markers, eng_segmentation, peak_mask, display_rgb
    ...
    def display_current_step(self):
        ...
        step_descriptions = [
            ...
            "Topography surface computed as Distance Transform (Valleys) or Morphological Gradient (Ridges), "
            "thresholded by the Foreground Peak Detection Cutoff Ratio to find candidate peaks.",
            "Watershed engine tuned with Peak Marker Minimum Separation Distance, Compactness and Connectivity "
            "Radius. Result seeds and refines the Markers/Watershed steps below.",
    ...
    def update_parameter_controls(self):
        ...
        elif step == 5:  # Topography & Distance Analytics (NEW)
            self.create_step5_controls()
        elif step == 6:  # Watershed Engine Optimization (NEW)
            self.create_step6_controls()
        elif step == 7:  # Markers
            self.create_step7_controls()
        elif step == 8:  # Watershed
            self.create_step8_controls()
        elif step == 9:  # Morphological Close
            self.create_step9_controls()
        elif step == 10:  # Morphological Open
            self.create_step10_controls()
        elif step == 11:  # Fill Holes
            self.create_step11_controls()
        elif step == 12:  # Labeling
            self.create_step12_controls()
        elif step == 13:  # Final
            self.create_step13_controls()
        elif step == 14:  # Perspective Warp
    ...
    def create_step1_controls(self):
        ...
        tk.Label(self.params_frame,
                 text="1. Click on the image to place polygon points.\n"
                      ...
                      "5. Click Next to run Steps 2-14.",
                 bg=self.PANEL, fg=self.FG2,
    ...
    def create_step5_controls(self):
        """Create controls for Step 5: Topography & Distance Analytics (NEW)"""
        skip_var = tk.BooleanVar(value=not self.params['step_5_topography']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_5_topography', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        ttk.Label(self.params_frame, text="Topography Surface Mode:").pack(anchor='w', pady=(5, 2))
        mode_var = tk.IntVar(value=self.params['step_5_topography']['mode'])
        ttk.Radiobutton(self.params_frame, text="Distance Transform (Valleys)",
                        variable=mode_var, value=0,
                        command=lambda: self.update_step5_mode(mode_var)).pack(anchor='w', padx=6, pady=2)
        ttk.Radiobutton(self.params_frame, text="Edge Morphological Gradient (Ridges)",
                        variable=mode_var, value=1,
                        command=lambda: self.update_step5_mode(mode_var)).pack(anchor='w', padx=6, pady=2)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        ttk.Label(self.params_frame, text="Foreground Peak Detection Cutoff Threshold Ratio:").pack(anchor='w', pady=(5, 2))
        fgt_frame = ttk.Frame(self.params_frame)
        fgt_frame.pack(fill='x', pady=2)
        fgt_var = tk.DoubleVar(value=self.params['step_5_topography']['fg_threshold'])
        ttk.Scale(fgt_frame, from_=0.0, to=0.95, orient='horizontal', variable=fgt_var,
                  command=lambda v: self.update_step5_fg_threshold(fgt_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        fgt_spinbox = ttk.Spinbox(fgt_frame, from_=0.0, to=0.95, increment=0.05, textvariable=fgt_var,
                                  width=6, command=lambda: self.update_step5_fg_threshold(fgt_var))
        fgt_spinbox.pack(side='right')
        fgt_spinbox.bind('<Return>', lambda e: self.update_step5_fg_threshold(fgt_var))
        fgt_spinbox.bind('<FocusOut>', lambda e: self.update_step5_fg_threshold(fgt_var))

        ttk.Label(self.params_frame,
                  text="Note: feeds the flooding surface used by Step 8 Watershed and the peak "
                       "candidates used by Step 6.",
                 wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)
        self.param_widgets = {'mode_var': mode_var, 'fgt_var': fgt_var}

    def create_step6_controls(self):
        """Create controls for Step 6: Watershed Engine Optimization (NEW)"""
        skip_var = tk.BooleanVar(value=not self.params['step_6_watershed_opt']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_6_watershed_opt', skip_var)).pack(anchor='w', pady=5)

        if not _SKIMAGE_AVAILABLE:
            ttk.Label(self.params_frame,
                      text="scikit-image not installed - Step 6 will pass through unchanged.",
                     wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        # Peak Marker Minimum Separation Distance
        ttk.Label(self.params_frame, text="Peak Marker Minimum Separation Distance:").pack(anchor='w', pady=(5, 2))
        mdist_frame = ttk.Frame(self.params_frame)
        mdist_frame.pack(fill='x', pady=2)
        mdist_var = tk.IntVar(value=self.params['step_6_watershed_opt']['min_distance'])
        ttk.Scale(mdist_frame, from_=1, to=100, orient='horizontal', variable=mdist_var,
                  command=lambda v: self.update_step6_min_distance(mdist_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        mdist_spinbox = ttk.Spinbox(mdist_frame, from_=1, to=100, textvariable=mdist_var,
                                    width=6, command=lambda: self.update_step6_min_distance(mdist_var))
        mdist_spinbox.pack(side='right')
        mdist_spinbox.bind('<Return>', lambda e: self.update_step6_min_distance(mdist_var))
        mdist_spinbox.bind('<FocusOut>', lambda e: self.update_step6_min_distance(mdist_var))

        # Watershed Compactness
        ttk.Label(self.params_frame, text="Watershed Compactness:").pack(anchor='w', pady=(10, 2))
        comp_frame = ttk.Frame(self.params_frame)
        comp_frame.pack(fill='x', pady=2)
        comp_var = tk.DoubleVar(value=self.params['step_6_watershed_opt']['compactness'])
        ttk.Scale(comp_frame, from_=0.0, to=5.0, orient='horizontal', variable=comp_var,
                  command=lambda v: self.update_step6_compactness(comp_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        comp_spinbox = ttk.Spinbox(comp_frame, from_=0.0, to=5.0, increment=0.05, textvariable=comp_var,
                                   width=6, command=lambda: self.update_step6_compactness(comp_var))
        comp_spinbox.pack(side='right')
        comp_spinbox.bind('<Return>', lambda e: self.update_step6_compactness(comp_var))
        comp_spinbox.bind('<FocusOut>', lambda e: self.update_step6_compactness(comp_var))

        # Connectivity Radius
        ttk.Label(self.params_frame, text="Connectivity Radius:").pack(anchor='w', pady=(10, 2))
        conn_frame = ttk.Frame(self.params_frame)
        conn_frame.pack(fill='x', pady=2)
        conn_var = tk.IntVar(value=self.params['step_6_watershed_opt']['connectivity'])
        ttk.Scale(conn_frame, from_=1, to=10, orient='horizontal', variable=conn_var,
                  command=lambda v: self.update_step6_connectivity(conn_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        conn_spinbox = ttk.Spinbox(conn_frame, from_=1, to=10, textvariable=conn_var,
                                   width=6, command=lambda: self.update_step6_connectivity(conn_var))
        conn_spinbox.pack(side='right')
        conn_spinbox.bind('<Return>', lambda e: self.update_step6_connectivity(conn_var))
        conn_spinbox.bind('<FocusOut>', lambda e: self.update_step6_connectivity(conn_var))

        ttk.Label(self.params_frame,
                  text="Note: engine output seeds the Step 7 Markers step below.",
                 wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)
        self.param_widgets = {'mdist_var': mdist_var, 'comp_var': comp_var, 'conn_var': conn_var}

    def create_step7_controls(self):
        """Create controls for Step 7: Histogram Markers"""
        skip_var = tk.BooleanVar(value=not self.params['step_7_markers']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_7_markers', skip_var)).pack(anchor='w', pady=5)
        ...
        low_var = tk.IntVar(value=self.params['step_7_markers']['low_threshold'])
        ttk.Scale(low_frame, from_=0, to=255, orient='horizontal', variable=low_var,
                  command=lambda v: self.update_step7_params(low_var, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        low_spinbox = ttk.Spinbox(low_frame, from_=0, to=255, textvariable=low_var,
                                  width=6, command=lambda: self.update_step7_params(low_var, None))
        low_spinbox.pack(side='right')
        low_spinbox.bind('<Return>', lambda e: self.update_step7_params(low_var, None))
        low_spinbox.bind('<FocusOut>', lambda e: self.update_step7_params(low_var, None))
        ...
        high_var = tk.IntVar(value=self.params['step_7_markers']['high_threshold'])
        ttk.Scale(high_frame, from_=0, to=255, orient='horizontal', variable=high_var,
                  command=lambda v: self.update_step7_params(None, high_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        high_spinbox = ttk.Spinbox(high_frame, from_=0, to=255, textvariable=high_var,
                                   width=6, command=lambda: self.update_step7_params(None, high_var))
        high_spinbox.pack(side='right')
        high_spinbox.bind('<Return>', lambda e: self.update_step7_params(None, high_var))
        high_spinbox.bind('<FocusOut>', lambda e: self.update_step7_params(None, high_var))
        self.param_widgets = {'low_var': low_var, 'high_var': high_var}

    def create_step8_controls(self):
        """Create controls for Step 8: Watershed"""
        skip_var = tk.BooleanVar(value=not self.params['step_8_watershed']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_8_watershed', skip_var)).pack(anchor='w', pady=5)
    ...
    def create_step9_controls(self):
        """Create controls for Step 9: Morphological Close"""
        skip_var = tk.BooleanVar(value=not self.params['step_9_morph_close']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_9_morph_close', skip_var)).pack(anchor='w', pady=5)
        ...
        iter_var = tk.IntVar(value=self.params['step_9_morph_close']['iterations'])
        ttk.Scale(iter_frame, from_=1, to=10, orient='horizontal', variable=iter_var,
                  command=lambda v: self.update_step9_iterations(iter_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        iter_spinbox = ttk.Spinbox(iter_frame, from_=1, to=10, textvariable=iter_var,
                                   width=6, command=lambda: self.update_step9_iterations(iter_var))
        iter_spinbox.pack(side='right')
        iter_spinbox.bind('<Return>', lambda e: self.update_step9_iterations(iter_var))
        iter_spinbox.bind('<FocusOut>', lambda e: self.update_step9_iterations(iter_var))
        self.param_widgets = {'iter_var': iter_var}

    def create_step10_controls(self):
        """Create controls for Step 10: Morphological Open"""
        skip_var = tk.BooleanVar(value=not self.params['step_10_morph_open']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_10_morph_open', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(self.params_frame, text="Number of Iterations:").pack(anchor='w', pady=(5, 2))
        iter_frame = ttk.Frame(self.params_frame)
        iter_frame.pack(fill='x', pady=2)
        iter_var = tk.IntVar(value=self.params['step_10_morph_open']['iterations'])
        ttk.Scale(iter_frame, from_=1, to=10, orient='horizontal', variable=iter_var,
                  command=lambda v: self.update_step10_iterations(iter_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        iter_spinbox = ttk.Spinbox(iter_frame, from_=1, to=10, textvariable=iter_var,
                                   width=6, command=lambda: self.update_step10_iterations(iter_var))
        iter_spinbox.pack(side='right')
        iter_spinbox.bind('<Return>', lambda e: self.update_step10_iterations(iter_var))
        iter_spinbox.bind('<FocusOut>', lambda e: self.update_step10_iterations(iter_var))
        self.param_widgets = {'iter_var': iter_var}

    def create_step11_controls(self):
        """Create controls for Step 11: Fill Holes"""
        skip_var = tk.BooleanVar(value=not self.params['step_11_fillholes']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_11_fillholes', skip_var)).pack(anchor='w', pady=5)

        ttk.Label(self.params_frame, text="Note: Fills small holes inside detected regions.",
                 wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)

    def create_step12_controls(self):
        """Create controls for Step 12: Labeling"""
        skip_var = tk.BooleanVar(value=not self.params['step_12_labeling']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_12_labeling', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(self.params_frame, text="Minimum Size (pixels):").pack(anchor='w', pady=(5, 2))
        size_frame = ttk.Frame(self.params_frame)
        size_frame.pack(fill='x', pady=2)
        size_var = tk.IntVar(value=self.params['step_12_labeling']['min_size'])
        ttk.Scale(size_frame, from_=0, to=200, orient='horizontal', variable=size_var,
                  command=lambda v: self.update_step12_size(size_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        size_spinbox = ttk.Spinbox(size_frame, from_=0, to=200, textvariable=size_var,
                                   width=6, command=lambda: self.update_step12_size(size_var))
        size_spinbox.pack(side='right')
        size_spinbox.bind('<Return>', lambda e: self.update_step12_size(size_var))
        size_spinbox.bind('<FocusOut>', lambda e: self.update_step12_size(size_var))
        self.param_widgets = {'size_var': size_var}

    def create_step13_controls(self):
        """Create controls for Step 13: Final Result"""
        ...
        min_area_var = tk.IntVar(value=self.params['step_13_final']['min_area'])
        ttk.Scale(min_area_frame, from_=0, to=1500, orient='horizontal', variable=min_area_var,
                  command=lambda v: self.update_step13_min_area(min_area_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        min_spinbox = ttk.Spinbox(min_area_frame, from_=0, to=1500, textvariable=min_area_var,
                                  width=6, command=lambda: self.update_step13_min_area(min_area_var))
        min_spinbox.pack(side='right')
        min_spinbox.bind('<Return>', lambda e: self.update_step13_min_area(min_area_var))
        min_spinbox.bind('<FocusOut>', lambda e: self.update_step13_min_area(min_area_var))
        ...
        max_area_var = tk.IntVar(value=self.params['step_13_final']['max_area'])
        ttk.Scale(max_area_frame, from_=1000, to=50000, orient='horizontal', variable=max_area_var,
                  command=lambda v: self.update_step13_max_area(max_area_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        max_spinbox = ttk.Spinbox(max_area_frame, from_=1000, to=50000, textvariable=max_area_var,
                                  width=6, command=lambda: self.update_step13_max_area(max_area_var))
        max_spinbox.pack(side='right')
        max_spinbox.bind('<Return>', lambda e: self.update_step13_max_area(max_area_var))
        max_spinbox.bind('<FocusOut>', lambda e: self.update_step13_max_area(max_area_var))
    ...
    def create_step14_controls(self):
        """Create controls for Step 14: Perspective Warp"""
        skip_var = tk.BooleanVar(value=not self.params['step_14_perspective']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                        variable=skip_var,
                        command=lambda: self.toggle_step_skip('step_14_perspective', skip_var)).pack(anchor='w', pady=5)
    ...
    def update_step5_mode(self, mode_var):
        """Update step 5 topography surface mode"""
        try:
            value = int(mode_var.get())
            self.params['step_5_topography']['mode'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            mode_var.set(self.params['step_5_topography']['mode'])

    def update_step5_fg_threshold(self, fgt_var):
        """Update step 5 foreground peak detection cutoff threshold ratio"""
        try:
            value = round(max(0.0, min(0.95, float(fgt_var.get()))), 3)
            fgt_var.set(value)
            self.params['step_5_topography']['fg_threshold'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            fgt_var.set(self.params['step_5_topography']['fg_threshold'])

    def update_step6_min_distance(self, mdist_var):
        """Update step 6 peak marker minimum separation distance"""
        try:
            value = max(1, min(100, int(mdist_var.get())))
            mdist_var.set(value)
            self.params['step_6_watershed_opt']['min_distance'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            mdist_var.set(self.params['step_6_watershed_opt']['min_distance'])

    def update_step6_compactness(self, comp_var):
        """Update step 6 watershed compactness"""
        try:
            value = round(max(0.0, min(5.0, float(comp_var.get()))), 3)
            comp_var.set(value)
            self.params['step_6_watershed_opt']['compactness'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            comp_var.set(self.params['step_6_watershed_opt']['compactness'])

    def update_step6_connectivity(self, conn_var):
        """Update step 6 connectivity radius"""
        try:
            value = max(1, min(10, int(conn_var.get())))
            conn_var.set(value)
            self.params['step_6_watershed_opt']['connectivity'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            conn_var.set(self.params['step_6_watershed_opt']['connectivity'])
    ...
    def tab2_next_step(self):
        ...
        if self.current_step == 1:
            if not self.step1_polygon_closed or len(self.step1_polygon_points) < 3:
                ...
                return
            # Run full pipeline (steps 2-14) now that ROI is confirmed
            self.process_all_steps()
            self.current_step = 2
            self.display_current_step()
            self.tab2_info_label.config(
    ...
    def tab3_load_config(self):
        ...
        if file_path:
            try:
                ...
                # Migrate old 12-step configs to the current 14-step key names
                self.tab3_config_data['pipeline_params'] = self._migrate_legacy_pipeline_params(
                    self.tab3_config_data['pipeline_params']
                )
    ...
    def tab3_process_single_image(self, image_path, defects_folder, non_defects_folder, filename):
        ...
        Mirrors process_all_steps (Single Image / Tab 2) exactly, including
        the new Step 5 (Topography & Distance Analytics) and Step 6
        (Watershed Engine Optimization) stages, so Batch and Single Image
        always run the same effective 14-step pipeline.
        """
        # Load image
        ...
        if self.tab3_config_data and 'pipeline_params' in self.tab3_config_data:
            params = self._migrate_legacy_pipeline_params(self.tab3_config_data['pipeline_params'])
        else:
            params = {
                'step_2_rotate_flip': {'enabled': True, 'rotate': 0, 'flip_h': False, 'flip_v': False},
                'step_3_grayscale': {'enabled': True},
        ...
        # Step 5: Topography & Distance Analytics (NEW)
        topo_surface, topo_peak_mask, _topo_disp = self._compute_topography_analytics(
            gray, params.get('step_5_topography', {'enabled': True, 'mode': 0, 'fg_threshold': 0.50})
        )

        # Step 6: Watershed Engine Optimization (NEW)
        eng_markers, eng_segmentation, eng_peak_mask, _eng_disp = self._compute_watershed_engine(
            gray, topo_surface, topo_peak_mask,
            params.get('step_6_watershed_opt',
                       {'enabled': True, 'min_distance': 5, 'compactness': 0.0, 'connectivity': 1})
        )
        ...
        if params.get('step_7_markers', {}).get('enabled', True):
            ...
            # Real feed-forward from Step 6 (see process_all_steps for the
            # matching Single Image logic)
            if eng_markers is not None:
                engine_fg = eng_markers > 1
                markers[engine_fg] = np.maximum(
                    markers[engine_fg],
        ...
        if params.get('step_8_watershed', {}).get('enabled', True):
            # Real feed-forward from Step 5: prefer the computed topography
            # surface as the flooding elevation map when available.
            if topo_surface is not None:
                elevation_map_uint8 = cv2.normalize(topo_surface, None, 0, 255,
                                                     cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            else:
        ...
        try:
            ...
            pass
        try:
            del markers, segmentation, binary_seg
        except Exception:
            pass
        try:
            del topo_surface, topo_peak_mask, eng_markers, eng_segmentation, eng_peak_mask
        except Exception:
            pass
        gc.collect()
...
class SplashScreen:
    ...
    def __init__(self, root):
        ...
        specs = [
            ...
            ('PIPELINE', '14-Step Analysis  \u00b7  ROI + Batch Processing'),
