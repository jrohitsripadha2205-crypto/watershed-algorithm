...
class UltimateWatershedWorkstation(ctk.CTk):
    ...
    def convert_to_ctk_image(self, cv_img, is_gray=False, use_cmap=None, labels_mask=None, orig_mask=None):
        """ Instantly normalizes array matrices into raw RGB bytes using high-speed OpenCV colormaps """
        if use_cmap == "jet" and labels_mask is not None:
            ...
            color_mapped[boundaries] = [0, 0, 255] # BGR conversion format
    ...
    # --- HIGH SPEED PURE OPENCV EXPORT ROUTINE ENGINE ---
    def background_export_worker(self, target_base_path):
        try:
            ...
            # 1. Save Parameter Settings (JSON)
            with open(os.path.join(output_dir, "parameters.json"), "w") as f:
                json.dump(export_config, f, indent=4)
                
            # 2. Save Raw Input & Binary Mask
            ...
            # 3. High-Speed Native C++ OpenCV Colormap Conversion (Topology Surface)
            ...
            # 4. High-Speed Segment Isolation with Red Outlines
            ...
            color_labels[boundaries] = [0, 0, 255] # Marks lines as bright red in native BGR format
