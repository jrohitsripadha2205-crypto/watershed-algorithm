...
# --- HIGH-PRECISION PURE OPENCV ENGINE ---
def process_universal_watershed(image_path, max_dim, blur_k, thresh_offset, morph_it, Use_Gradient, fg_thresh, min_dist, compactness, connectivity):
    ...
    # Fast Resize Strategy for Live UI Responsiveness
    h, w = img.shape[:2]
    ...
    # Generate Topography Landscape using pure OpenCV
    if Use_Gradient > 0:
        gradient_surface = cv2.morphologyEx(gray, cv2.MORPH_GRADIENT, cv2.getStructuringElement(cv2.MORPH_RECT, (3,3)))
        display_landscape = gradient_surface.copy()
        peak_surface = cv2.bitwise_not(gradient_surface)
        peak_surface[cleaned_mask == 0] = 0
    else:
        # Distance Transformation Valley Flooding Map
        distance_map = cv2.distanceTransform(cleaned_mask, cv2.DIST_L2, 5)
        display_landscape = distance_map.copy()
        peak_surface = distance_map.copy()
        max_dist_val = distance_map.max()
        if max_dist_val > 0:
            peak_surface[distance_map < (fg_thresh * max_dist_val)] = 0

    # Pure OpenCV Peak Threshold Marker Extraction Loop
    _, peak_mask = cv2.threshold(peak_surface, 0.5 * peak_surface.max() if peak_surface.max() > 0 else 0, 255, cv2.THRESH_BINARY)
    peak_mask = np.uint8(peak_mask)
    
    # OpenCV Morphological Dilation using the 'min_dist' parameter to control peak separation
    if min_dist > 1:
        dilation_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (int(min_dist), int(min_dist)))
        peak_mask = cv2.dilate(peak_mask, dilation_kernel)

    # Label isolated object segments using pure OpenCV connected components
    ret, markers = cv2.connectedComponents(peak_mask)
    
    # OpenCV Watershed Rule: The unknown region must be 0, and sure background must be 1.
    markers = markers + 1
    unknown_zone = cv2.subtract(cleaned_mask, peak_mask)
    markers[unknown_zone == 255] = 0
    
    # Run the pure OpenCV Watershed algorithm
    # OpenCV modifies the markers array in place and requires a 3-channel BGR source frame image.
    cv2.watershed(img_disp, markers)
    
    return img_disp, cleaned_mask, display_landscape, markers
...
class UltimateWatershedWorkstation(ctk.CTk):
    def __init__(self):
        ...
        self.title("Universal OpenCV Watershed Prototyping Workstation")
    ...
    def build_tabs_layout(self):
        ...
        # NOTE: In pure OpenCV watershed, compactness and connectivity configurations do not alter 
        # the standard C++ implementation layout, so they function as code metadata values for comparison.
        ...
        self.v_comp, self.s_comp = self.create_parameter_row(self.tab_ws, "[OpenCV Comparison Check] Compactness Parameter:", 0.0, 5.0, 0.00, 1, is_float=True, resolution=0.05)
        self.v_conn, self.s_conn = self.create_parameter_row(self.tab_ws, "[OpenCV Comparison Check] Connectivity Metric:", 1, 10, 1, 2, resolution=1)
    ...
    def convert_to_ctk_image(self, cv_img, is_gray=False, use_cmap=None, labels_mask=None, orig_mask=None):
        if use_cmap == "jet" and labels_mask is not None:
            # OpenCV marks watershed boundary border lines exactly as -1 pixels
            labels_norm = cv2.normalize(labels_mask, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            color_mapped = cv2.applyColorMap(labels_norm, cv2.COLORMAP_JET)
            color_mapped[labels_mask == -1] = [0, 0, 255] # Explicitly draw boundary lines in red (BGR format)
            ...
            cv_cmap = cv2.COLORMAP_MAGMA if use_cmap == "magma" else cv2.COLORMAP_VIRIDIS
            color_mapped = cv2.applyColorMap(norm_topo, cv_cmap)
            rgb = cv2.cvtColor(color_mapped, cv2.COLOR_BGR2RGB)
        elif is_gray:
    ...
    def background_export_worker(self, target_base_path):
        try:
            ...
            output_dir = os.path.join(target_base_path, f"watershed_opencv_{base_filename}_outputs")
            ...
            export_config = {
                "algorithm_metadata": "pure_opencv_universal_workstation_pipeline",
                ...
                "optimized_parameters": {
                    ...
                    "watershed_peak_min_distance_dilation": mdist
                }
            }
            ...
            color_labels = cv2.applyColorMap(labels_norm, cv2.COLORMAP_JET)
            color_labels[full_labels == -1] = [0, 0, 255] 
            cv2.imwrite(os.path.join(output_dir, "export_4_segmentation.png"), color_labels)
