...
"""
    ...
    def init_tab2(self):
        ...
        self.params = {
            ...
            'step_5_gaussian_division': {'enabled': True, 'sigma': 25},
            'step_6_batch_segmentation': {'enabled': True, 'threshold_offset': 50, 'morph_iterations': 1},
    ...
    def _migrate_pipeline_params(self, params):
        ...
        migrated.setdefault('step_5_gaussian_division', {'enabled': True, 'sigma': 25})
        migrated.setdefault('step_6_batch_segmentation',
                             {'enabled': True, 'threshold_offset': 50, 'morph_iterations': 1})
        return migrated
    ...
    def process_all_steps(self):
        ...
        # Step 5: Gaussian Division (illumination normalization)  [NEW]
        if self.params['step_5_gaussian_division']['enabled']:
            sigma = max(1, int(self.params['step_5_gaussian_division']['sigma']))
            gray_f = gray.astype(np.float32)
            illumination = cv2.GaussianBlur(gray_f, (0, 0), sigmaX=sigma, sigmaY=sigma)
            illumination[illumination < 1e-6] = 1e-6
            divided = (gray_f / illumination) * 128.0
            gdiv = np.clip(divided, 0, 255).astype(np.uint8)
        else:
            gdiv = gray.copy()
        self.step_images[5] = cv2.cvtColor(gdiv, cv2.COLOR_GRAY2RGB)
        self.step_data['gdiv'] = gdiv

        # Step 6: Batch-style Dual-Threshold + Watershed Segmentation  [NEW]
        # (adapted from batch_processing.py's process_powder_bed_pipeline)
        if self.params['step_6_batch_segmentation']['enabled']:
            offset = self.params['step_6_batch_segmentation']['threshold_offset']
            morph_it = max(0, int(self.params['step_6_batch_segmentation']['morph_iterations']))

            blur_k = 5
            blurred2 = cv2.GaussianBlur(gdiv, (blur_k, blur_k), 0)

            bright_cutoff = int(np.clip(128 - offset, 1, 254))
            dark_cutoff = int(np.clip(128 + offset, 1, 254))
            _, bright_mask2 = cv2.threshold(blurred2, bright_cutoff, 255, cv2.THRESH_BINARY)
            _, dark_mask2 = cv2.threshold(blurred2, dark_cutoff, 255, cv2.THRESH_BINARY_INV)
            combined_binary = cv2.bitwise_or(bright_mask2, dark_mask2)

            selem = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
            cleaned_mask = cv2.morphologyEx(combined_binary, cv2.MORPH_OPEN, selem, iterations=morph_it) \
                if morph_it > 0 else combined_binary

            num_labels2, labels_im2, stats2, _ = cv2.connectedComponentsWithStats(cleaned_mask)
            new_segmented_mask = np.zeros_like(cleaned_mask)
            for i in range(1, num_labels2):
                area = stats2[i, cv2.CC_STAT_AREA]
                if 50 <= area <= 10000:
                    new_segmented_mask[labels_im2 == i] = 255

            flooding_surface = -cv2.distanceTransform(new_segmented_mask, cv2.DIST_L2, 5)
            ret2, markers2 = cv2.connectedComponents(new_segmented_mask)
            markers2 = markers2 + 1
            unknown2 = cv2.subtract(cv2.dilate(new_segmented_mask, selem, iterations=1), new_segmented_mask)
            markers2[unknown2 == 255] = 0
            try:
                _ = watershed(flooding_surface, markers2, mask=new_segmented_mask)
            except Exception:
                pass  # watershed result itself isn't needed downstream; mask is what propagates

            batch_seg_display = cv2.cvtColor(gdiv, cv2.COLOR_GRAY2RGB)
            batch_seg_display[new_segmented_mask > 0] = [255, 0, 0]
        else:
            new_segmented_mask = np.zeros_like(gdiv)
            batch_seg_display = cv2.cvtColor(gdiv, cv2.COLOR_GRAY2RGB)
        ...
        if not self.params['step_7_markers']['enabled'] and not self.params['step_8_watershed']['enabled']:
            ...
            # inserted Step 5 (Gaussian Division) / Step 6 (batch threshold +
            # watershed) instead of a bare passthrough, so the pipeline never
    ...
    def display_current_step(self):
        ...
        step_names = [
            ...
            "Step 5: Gaussian Division (Illumination Normalization)",
            "Step 6: Threshold + Watershed Segmentation",
        ...
        step_descriptions = [
            ...
            "Gaussian Division normalizes uneven illumination by dividing the image by a heavily-blurred "
            "version of itself, improving contrast without degrading defect boundaries.",
            "Dual-threshold binary segmentation (bright + dark) with morphological cleanup and watershed, "
            "adapted from the batch powder-bed pipeline. Produces the segmented mask that downstream "
            "steps use automatically if Markers/Watershed below are skipped.",
    ...
    def update_parameter_controls(self):
        ...
        elif step == 5:  # Gaussian Division
            self.create_step5_controls()
        elif step == 6:  # Batch-style Threshold + Watershed Segmentation
    ...
    def create_step5_controls(self):
        """Create controls for Step 5: Gaussian Division (illumination normalization)"""
        skip_var = tk.BooleanVar(value=not self.params['step_5_gaussian_division']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_5_gaussian_division', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(self.params_frame, text="Sigma (illumination blur radius):").pack(anchor='w', pady=(5, 2))
        sigma_frame = ttk.Frame(self.params_frame)
        sigma_frame.pack(fill='x', pady=2)
        sigma_var = tk.IntVar(value=self.params['step_5_gaussian_division']['sigma'])
        ...
        self.param_widgets = {'sigma_var': sigma_var}

        ttk.Label(self.params_frame,
                  text="Note: divides the image by a heavily-blurred copy of itself to normalize "
                       "uneven illumination while preserving small defect boundaries.",
                  wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)

    def create_step6_controls(self):
        """Create controls for Step 6: Threshold + Watershed Segmentation (from batch pipeline)"""
        skip_var = tk.BooleanVar(value=not self.params['step_6_batch_segmentation']['enabled'])
        ttk.Checkbutton(self.params_frame, text="Skip this step",
                       variable=skip_var,
                       command=lambda: self.toggle_step_skip('step_6_batch_segmentation', skip_var)).pack(anchor='w', pady=5)

        ttk.Separator(self.params_frame, orient='horizontal').pack(fill='x', pady=10)

        ttk.Label(self.params_frame, text="Threshold Offset (bright/dark split):").pack(anchor='w', pady=(5, 2))
        offset_frame = ttk.Frame(self.params_frame)
        offset_frame.pack(fill='x', pady=2)
        offset_var = tk.IntVar(value=self.params['step_6_batch_segmentation']['threshold_offset'])
        ttk.Scale(offset_frame, from_=-120, to=120, orient='horizontal', variable=offset_var,
                  command=lambda v: self.update_step6_params(offset_var, None)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        offset_spinbox = ttk.Spinbox(offset_frame, from_=-120, to=120, textvariable=offset_var,
                                     width=6, command=lambda: self.update_step6_params(offset_var, None))
        offset_spinbox.pack(side='right')
        offset_spinbox.bind('<Return>', lambda e: self.update_step6_params(offset_var, None))
        offset_spinbox.bind('<FocusOut>', lambda e: self.update_step6_params(offset_var, None))

        ttk.Label(self.params_frame, text="Morphological Cleanup Iterations:").pack(anchor='w', pady=(10, 2))
        morph_frame = ttk.Frame(self.params_frame)
        morph_frame.pack(fill='x', pady=2)
        morph_var = tk.IntVar(value=self.params['step_6_batch_segmentation']['morph_iterations'])
        ttk.Scale(morph_frame, from_=0, to=15, orient='horizontal', variable=morph_var,
                  command=lambda v: self.update_step6_params(None, morph_var)).pack(side='left', fill='x', expand=True, padx=(0, 5))
        morph_spinbox = ttk.Spinbox(morph_frame, from_=0, to=15, textvariable=morph_var,
                                    width=6, command=lambda: self.update_step6_params(None, morph_var))
        morph_spinbox.pack(side='right')
        morph_spinbox.bind('<Return>', lambda e: self.update_step6_params(None, morph_var))
        morph_spinbox.bind('<FocusOut>', lambda e: self.update_step6_params(None, morph_var))
        self.param_widgets = {'offset_var': offset_var, 'morph_var': morph_var}

        ttk.Label(self.params_frame,
                  text="Note: dual-threshold binary segmentation with morphological cleanup and watershed, "
                       "adapted from the batch powder-bed pipeline. If Markers (Step 7) and Watershed "
                       "(Step 8) below are both skipped, this step's segmented mask is forwarded "
                       "automatically so the pipeline never receives an empty image.",
                  wraplength=280, font=(self.FONT, 8)).pack(anchor='w', pady=10)
    ...
    def update_step5_sigma(self, sigma_var):
        ...
        try:
            value = max(1, min(150, int(sigma_var.get())))
            sigma_var.set(value)
            self.params['step_5_gaussian_division']['sigma'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            sigma_var.set(self.params['step_5_gaussian_division']['sigma'])

    def update_step6_params(self, offset_var, morph_var):
        """Update step 6 batch-style threshold offset / morphological iterations"""
        try:
            if offset_var:
                value = max(-120, min(120, int(offset_var.get())))
                offset_var.set(value)
                self.params['step_6_batch_segmentation']['threshold_offset'] = value
            if morph_var:
                value = max(0, min(15, int(morph_var.get())))
                morph_var.set(value)
                self.params['step_6_batch_segmentation']['morph_iterations'] = value
            self.process_all_steps()
            self.display_current_step()
        except ValueError:
            if offset_var:
                offset_var.set(self.params['step_6_batch_segmentation']['threshold_offset'])
            if morph_var:
                morph_var.set(self.params['step_6_batch_segmentation']['morph_iterations'])
    ...
    def tab3_process_single_image(self, image_path, defects_folder, non_defects_folder, filename):
        ...
        # Step 5: Gaussian Division (illumination normalization)  [NEW]
        if params.get('step_5_gaussian_division', {}).get('enabled', True):
            sigma = max(1, int(params['step_5_gaussian_division'].get('sigma', 25)))
            gray_f = gray.astype(np.float32)
            illumination = cv2.GaussianBlur(gray_f, (0, 0), sigmaX=sigma, sigmaY=sigma)
            illumination[illumination < 1e-6] = 1e-6
            gdiv = np.clip((gray_f / illumination) * 128.0, 0, 255).astype(np.uint8)
        else:
            gdiv = gray.copy()

        # Step 6: Batch-style Dual-Threshold + Watershed Segmentation  [NEW]
        if params.get('step_6_batch_segmentation', {}).get('enabled', True):
            offset = params['step_6_batch_segmentation'].get('threshold_offset', 50)
            morph_it = max(0, int(params['step_6_batch_segmentation'].get('morph_iterations', 1)))

            blurred2 = cv2.GaussianBlur(gdiv, (5, 5), 0)
            bright_cutoff = int(np.clip(128 - offset, 1, 254))
            dark_cutoff = int(np.clip(128 + offset, 1, 254))
            _, bright_mask2 = cv2.threshold(blurred2, bright_cutoff, 255, cv2.THRESH_BINARY)
            _, dark_mask2 = cv2.threshold(blurred2, dark_cutoff, 255, cv2.THRESH_BINARY_INV)
        ...
        if not params.get('step_7_markers', {}).get('enabled', True) and \
           not params.get('step_8_watershed', {}).get('enabled', True):
            ...
            # (Gaussian Division + batch threshold/watershed) instead of an
